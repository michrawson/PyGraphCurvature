...
def compare_curvatures(G, G_fcurv, G_fcurv_alt, LLY_curvatures):
    ...
    plt.hist(G_fcurv[np.triu_indices(G_fcurv.shape[0])].flatten(),bins=np.arange(np.nanmin(G_fcurv.flatten()),
                                              1+np.nanmax(G_fcurv.flatten())),rwidth=0.85,color='skyblue')#,edgecolor='black')
    # plt.title('histogram')
    plt.xlabel('Forman curvature')
    plt.ylabel('count')
    plt.show()

    plt.hist(G_fcurv_alt[np.triu_indices(G_fcurv_alt.shape[0])].flatten(),bins=np.arange(np.nanmin(G_fcurv_alt.flatten()),
                                                  1+np.nanmax(G_fcurv_alt.flatten())),rwidth=0.85,color='skyblue')#,edgecolor='black')
    # plt.title('histogram')
    plt.xlabel('Forman aug. curvature')
    plt.ylabel('count')
    plt.show()
        
    plt.hist(LLY_curvatures[np.triu_indices(LLY_curvatures.shape[0])].flatten(),bins=40,rwidth=0.85,color='skyblue')#,edgecolor='black')
    # plt.title('histogram')
...
def curv_vs_time(graph_data, n_range, alpha):
    ...
    for index in range(len(n_range)):
        ...
        G_fcurv_alt = forman_aug_curvature(G)
